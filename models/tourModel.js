...
const tourSchema = new mongoose.Schema(
  {
    ...
    duration: {
      type: Number,
      required: [true, 'A tour mush must have a duration'],
    },
    maxGroupSize: {
      type: Number,
      required: [true, 'A tour mush must have a group size'],
    },
    difficulty: {
      type: String,
      required: [true, 'A tour mush must have a difficulty'],
      enum: {
...
tourSchema.post(/^find/, function (docs, next) {
  console.log(`This request took ${Date.now() - this.start} in milliseconds!!`);
  next();
});
